...
The repository is the public `OrbitSystemsAi/orbit-news-network` repository owned by the verified Orbit Systems AI organization. Public visibility was explicitly approved on 2026-07-27 to support Vercel Hobby Git integration. Never substitute a personal repository, change visibility again, overwrite an existing repository, replace remotes, force-push, or rewrite history without explicit approval.
...
6. Create it only when absent, with the visibility explicitly approved by the repository owner.
...
No GitHub Actions deployment workflow is required for the MVP because Vercel Git integration provides the deployment path. If CI is added later, keep it lightweight and never migrate or seed a database from pull-request workflows.
