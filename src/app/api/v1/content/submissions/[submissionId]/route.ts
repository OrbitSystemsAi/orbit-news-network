...
import { toContentSubmissionResponse } from "@/server/services/content-contract";
...
export async function GET(request:Request,{params}:{params:Promise<{submissionId:string}>}){
 ...
 if(item.projectId===project.id)return apiSuccess(toContentSubmissionResponse(item));
